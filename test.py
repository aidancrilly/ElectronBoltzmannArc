...
E_applied,n_neutral,vmax,Nv = 1e3,1e24,1.5e7,8
Solver = EBA_solver(E_applied,n_neutral,vmax,Nv,mu,mu_w)
...
for i in range(Nt):
    ...
    fig = plt.figure(dpi=200,figsize=(8.5,5))
    ax1 = fig.add_subplot(221)
    f0 = np.sum(Solver.fe*Solver.mu_w[:,None],axis=0)
    f1 = np.sum(Solver.fe*Solver.mu_w[:,None]*Solver.mu_arr[:,None],axis=0) 
    ax1.plot(Solver.v_arr/1e6,f0/np.amax(f0),label='f0')
    ax1.plot(Solver.v_arr/1e6,f1/np.amax(f0),label='f1')
    ax1.legend()
    ax1.set_xlim(0.0,vmax/1e6)
